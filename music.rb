...
class Playlist
  def initialize(name, tracks = [], current_track_index = 1)
    @name = name
    @tracks = tracks
  ...
  def play_track(track_number)
    ...
    "Now playing #{track_number} - #{track.name} by #{track.artist}, #{track.length}"
  end
...
# # play some tracks
# puts playlist.play_track(1) # Now playing 1 - all over by joyner lucas, 4:43
# puts playlist.play_track(4) # Now playing 4 - 44 bars by logic, 3:02
...
puts playlist.play_current_track # Now playing 1 - all over by joyner lucas, 4:43
playlist.next_track
playlist.next_track

puts playlist.play_current_track # Now playing 3 - ross capichionni by joyner lucas, 7:40

playlist.previous_track
puts playlist.play_current_track # Now playing 2 - opposites attract by joyner lucas, 4:33
